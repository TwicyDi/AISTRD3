...
void List<T>::make_group(Node *first, Node *last)
{
	...
	{
		Node* separation = nullptr;
		size_t sum = 0;
		size_t check = counting_average_frequancy(first, last);
		for (Node* cur = first; cur != last->next; cur = cur->next)
		{
			sum += cur->freq;
			if (sum <= check || separation == nullptr)
			{
				cur->code += '0';
				separation = cur;
			}
		...
		make_group(first, separation);
		make_group(separation->next, last);
	}
}
